...
from models.heads.detection.experimental.anchor_generator import AnchorGenerator
...
class PseudoSampler:
    ...
    def sample(self, assign_result, bboxes, gt_bboxes, *args, **kwargs):
        ...
        sampling_result = (pos_inds, neg_inds, bboxes, gt_bboxes, assign_result, gt_flags)
...
class StandardRoIHead(BaseRoIHead):
    ...
    def init_assigner_sampler(self) -> None:
        ...
        if self.train_cfg:
            ...
            self.bbox_sampler = PseudoSampler(**self.train_cfg.sampler)

    def init_bbox_head(self, bbox_roi_extractor, bbox_head) -> None:
...
class DetectionHead(nn.Module):
    def __init__(self, args, num_classes) -> None:
        super().__init__()
        channels = 256
        self.neck = FPN(in_channels=[48, 96, 224, 448], out_channels=channels, num_outs=4)
        self.rpn_head = RPNHead(in_channels=channels)
    ...
    def forward(self, x):
        ...
        print(len(out), [x.size() for x in out])
        return out
    
def efficientformer_detection_head(feature_dim, num_classes):
    return DetectionHead(args=None, num_classes=num_classes)
