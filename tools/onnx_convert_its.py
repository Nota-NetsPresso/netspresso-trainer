...
def parse_args():
    ...
    parser.add_argument(
        '-c', '--config-path', type=str, default="/app/netspresso-trainer/tools/model_eval.yaml",
        help="Model config path")
    parser.add_argument(
        '-o', '--output-dir', type=str, default="/app/netspresso-trainer/tools/innovation",
        help="ONNX model output path")
    parser.add_argument(
        '--sample-size', type=int, nargs=2, default=[736, 1280],
        help="Model input size (height, width). Must be multiples of 32 for YOLO models.")
    parser.add_argument(
        '--debug', action='store_true', help="Debug mode to check with the error message")
    parser.add_argument(
        '--num_classes', type=int, default=28,
